(function($) {
	...
	function setDefaultCombo() {
		for (var i = 0; i < count_day_on; i++) {
			var value = list_day_on[i];
			var arr = value.split("/");
			var month = arr[1];
			var day = arr[2];
			...
			for (let index = 0; index < arr_day.length; index++) {
				...
				if (check_day == day && !arr_day[index].classList.contains('disabled')) {
					var obj_dd = $(arr_day[index]).attr("data-date");
					$("#date_" + month + " [data-date=" + obj_dd + "]")
						.removeClass('active')
	...
	function ClickToggleClass() {
		for (var index = 1; index <= 12; index++) {
			...
			$("#date_" + index + " .table-condensed td.day").on('click',
				function(e) {
					e.preventDefault();
					...
					// check neu class old return false
					if ($(this).hasClass('new') || $(this).hasClass('old')) {
						return false;
					}
					// lấy tháng hiện tại
					var year_change = $("#input-current__date")
						.val(); // năm thay đổi
					...
					if (year_change > year_now) {
						...
						year_now
					) { // kiểm tra năm change mà nhỏ hơn năm hiện tai thì báo lỗi
						...
						for (let index = 0; index < obj_day
							.length; index++) {
							var check_day = obj_day[index].innerText;
							if (check_day ==
								day_now
							) // kiểm tra nếu ngày trong tháng bằng với ngày hiện tại
							{
								var data_date_now = $(obj_day[index])
									.attr(
										"data-date"
									); // lấy data data-date
							}
						}
						var data_date_click = $(this).attr(
							"data-date"
						); // lấy data-date của ngày đang click
						if (data_date_now <
							data_date_click
						) { // so sánh nếu date-date hiện tại nhỏ hơn data date click thì add class
							if (!$(this).hasClass('disabled')) {
								$(this).toggleClass('active-date')
									.removeClass('active');
							}
	...
	function changeYear(year) {
		$.ajax({
			...
			success: function(html) {
				if (html.status == true) {
					list_day_on = html.data.list_day_on;
					count_day_on = list_day_on.length;
					setDefaultCombo();
				}
			},
			complete: function() {
